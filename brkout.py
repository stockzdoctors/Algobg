...
class CandleBreakoutStrategy:
    ...
    def analyze(self, df, symbol, date_tracker=None):
        ...
        # --- FIX: DETECT TODAY'S DATE AND FILTER ---
        ...
        for i in range(len(df)):
            time_val = df.index[i]
            # Match both Today's Date AND 09:15 Time
        ...
        # Check all candles AFTER the 9:15 candle for breakout
        for i in range(reference_idx + 1, len(df)):
            current_candle = df.iloc[i]
            
            # Ensure we are only looking at candles from TODAY
...
def check_for_new_signals(selected_symbols, timeframe, strategy_name, risk_amount, mode, existing_signals, **strategy_params):
    ...
    for symbol in selected_symbols:
        data = fetch_data(symbol, timeframe, n_bars=100) # Increased bars to ensure today's 9:15 is found
